...
const InfoItem = ({ item, index, step = 0 }: any) => {
  ...
  return (
    <div
      key={index}
      className={`group relative flex items-center rounded-sm px-2 text-zinc-600 first:pl-0 ${
      ...
      <div className={'flex items-start justify-between text-zinc-600'}>
        ...
        <div className={'flex-1 space-y-1'}>
          <DivInput
            ...
            className={`w-full !py-0 text-[26px] font-semibold`}
          ...
          <div className={'border-b border-b-zinc-100'}>
            {(!readOnly || !!resumeData.content.config.desc) && (
              <DivInput
                placeholder={'其他信息'}
                className={`w-full !py-0 text-[17px] text-zinc-600`}
          ...
          <div
            className={'mt-2 flex items-center divide-x divide-solid divide-zinc-200 rounded-sm bg-zinc-50 px-2 py-1.5'}
          >
            {resumeData.content.baseInfo.list.slice(1, 7).map((item, index) => {
